...
const ContentSection = ({ title, onBack, course }: ContentSectionProps) => {
  ...
  const titleMap = {
    "Antes de clase": "Aula Invertida", //"Prebriefing",
    "Durante la clase": "Taller de Habilidad", //"Briefing",
    "Después de la clase": "Experiencia de Alta Fidelidad", //"Debriefing",
  } as const;
  ...
  useEffect(() => {
    ...
    const url = `${process.env.NEXT_PUBLIC_API_URL}/api/grades/student/${encodeURIComponent(
      username
  ...
  useEffect(() => {
    if (!username || !course?.courseId) return;
    const url = `${process.env.NEXT_PUBLIC_API_URL}/api/progress/course/${encodeURIComponent(
      course.courseId
  ...
  const updateProgress = (updatedSection: any) => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/progress`, {
      method: "POST",
      headers: {
                              ...
                              <>
                                ...
                                <div className="pt-2">
                                  <label className="inline-flex items-center gap-2 text-sm text-primary-30">
                                    <input
                                      type="checkbox"
                                      checked={completedContents[globalIndex] || false}
                                      onChange={() => toggleCompleted(globalIndex)}
                                    />
                                    Marcar como completado
                                  </label>
                                </div>
