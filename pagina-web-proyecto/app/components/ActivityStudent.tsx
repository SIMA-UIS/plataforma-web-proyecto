...
const ActivityStudent: React.FC<ActivityStudentProps> = ({
    ...
    const fetchProgress = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/progress/course/${id}/student/${username}/percentage`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  return (
    <div
      onClick={onClick}
      className="bg-white border border-gray-600/10 shadow-md rounded-lg overflow-hidden w-full max-w-sm transition-shadow hover:shadow-lg cursor-pointer"
    >
      <div
