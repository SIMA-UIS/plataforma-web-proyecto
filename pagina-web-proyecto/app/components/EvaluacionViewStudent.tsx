    ...
    const fetchGrade = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/grades/student/${username}/course/${course.courseId}`,
          {
            method: "GET",
            headers: {
  ...
  const saveGrade = async (sectionGrade: SectionGrade) => {
    try {
      ...
      if (putRes.status === 404) {
        const postRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/grades`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
