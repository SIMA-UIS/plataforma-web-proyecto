...
const getUserName = async (email: string): Promise<{ firstname: string; lastname: string }> => {
  try {
    const d = await fetchJSON(`${process.env.NEXT_PUBLIC_API_URL}/api/users/${encodeURIComponent(email)}`);
    if (Array.isArray(d)) {
...
const getGrade = async (email: string, courseId: string): Promise<Grade | null> => {
  const url = `${process.env.NEXT_PUBLIC_API_URL}/api/grades/student/${encodeURIComponent(email)}/course/${courseId}`;
  try {
    const res = await fetch(url, { headers: authHeaders });
...
export default function Feedback({ course, onClose }: { course: Course | null; onClose: () => void }) {
  ...
  const onSaveFeedback = async () => {
    ...
    try {
      ...
      let url = `${process.env.NEXT_PUBLIC_API_URL}/api/grades`;
      let method: "POST" | "PUT" = "POST";
      if (current.id) {
        url = `${process.env.NEXT_PUBLIC_API_URL}/api/grades/${current.id}`;
        method = "PUT";
      }
